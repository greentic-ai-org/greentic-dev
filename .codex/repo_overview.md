...
- **Path:** `src/dw_cmd.rs`, `src/distributor.rs`
  ...
  **Key functionality:** `dw_cmd.rs` implements `greentic-dev dw publish` / `dw install` against a greentic-store-server. `publish` reads a describe JSON + a `.gtpack`, computes the artifact sha256 locally, builds the `{describe, artifactSha256}` metadata envelope, and POSTs multipart (`metadata` + `artifact`) to `/api/v1/agentic-workers` with `Authorization: Bearer <token>` (token precedence: `--token` flag → `GREENTIC_STORE_TOKEN` env). `install` GETs `/api/v1/agentic-workers/{name}/{version}` for the expected `artifactSha256`, downloads `.../artifact`, verifies sha256 (hard fail, no file written on mismatch), and writes `{out|.}/{name}-{version}.gtpack` atomically (temp + rename). Pure builders (`build_publish_metadata`, `sha256_hex`, `resolve_publish_token`, `extract_artifact_sha256`, `encode_path_segment`) are unit-tested; the HTTP flows are covered by httpmock integration tests in `tests/dw_store_client.rs`. `distributor.rs` is the older distributor-resolve HTTP client.
